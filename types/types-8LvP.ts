...
// 日期工具函数
export const formatDate = (date: string | Date, locale: 'zh' | 'en'): string => {
...
// 数据分析函数
export const calculatePainStatistics = (entries: PainEntry[]): PainStatistics => {
  if (entries.length === 0) {
    return {
      totalEntries: 0,
      averagePain: 0,
...
// 本地存储工具函数
export const getStorageKey = (dataType: string, userId?: string): string => {
